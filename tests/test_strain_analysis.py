...
def test_normalize_points_by_scale():
    ...
    normalized = normalize_points_by_scale(fiducial_points, scale_points, scale_length)
    assert np.isclose(normalized[0, 0], -10.0)  # First point should be shifted left by scale_length
...
def test_calculate_strain_tensor():
    ...
    strain_large = calculate_strain_tensor(original, deformed, strain_type="green_lagrangian")
    ...
    with pytest.raises(ValueError):
        calculate_strain_tensor(original, deformed, center_index=5)  # Invalid center index
