...
def normalize_points_by_scale(
    ...
    if fiducial_points.shape != (5, 2):
        raise ValueError("fiducial_points must be an array of shape (5,2) with exactly 5 points")
    if scale_points.shape != (2, 2):
        raise ValueError("scale_points must be an array of shape (2,2) with exactly 2 points")
...
def calculate_strain_tensor(
    ...
    if center_index < 0 or center_index >= len(original_points):
        raise ValueError(f"center_index must be between 0 and {len(original_points)-1}")
    ...
    if strain_type == "small":
        ...
        strain_tensor = 0.5 * (F + F.T) - I
    else:
        # Green-Lagrangian strain (for large deformations)
        strain_tensor = 0.5 * (F.T @ F - I)
